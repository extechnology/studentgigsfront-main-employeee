...
import { Check, ChevronRight, AlertTriangle, User, MapPin, Calendar, CheckCircle2, Mail } from 'lucide-react';
...
const enrollmentSchema = z.object({
  ...
  whatsapp: z.string({ required_error: "WhatsApp Number is required" })
    .refine((value) => isValidPhoneNumber(value || ""), {
  ...
  if (data.status === 'School Student') {
    ...
    if (!data.parentPhone || !isValidPhoneNumber(data.parentPhone)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Valid Parent Phone Number is required',
        path: ['parentPhone'],
      });
    }
...
const EnrollmentModal: React.FC<EnrollmentModalProps> = ({ children, course_id }) => {
  ...
  // Submit Form Hook
  const { mutate: FormSubmit, isPending } = SubmitEnroll()
  ...
  const onSubmit = async (data: EnrollmentFormValues) => {
    ...
    // payload create for enroll form
    const payload = {
      ...
      city: data.city,
      are_you_a: data.status,
      parent_name: data.parentName || "",
      parent_phone: data.parentPhone || "",
      terms_one: data.informParent || false,
      terms_two: data.safetyCheck || false,
      course_id: course_id
    };
    ...
    // Submit Form and payment processing
    FormSubmit(formData, {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <Controller
                          ...
                          render={({ field }) => (
                            <PhoneInput
                              ...
                              className="flex h-11 w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-within:outline-none focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                            />
                          )}
                        />
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <Input id="email" placeholder="[EMAIL_ADDRESS]" className="pl-10 bg-white border-gray-200 h-11 rounded-xl" {...register('email')} />
                      </div>
                            ...
                            <Controller
                              ...
                              render={({ field }) => (
                                <PhoneInput
                                  ...
                                  value={field.value}
                                  onChange={field.onChange}
                                  className="flex h-11 w-full rounded-xl border border-orange-200 bg-white px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-within:outline-none focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                                />
                              )}
                            />
